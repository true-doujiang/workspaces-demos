package com.yhh.hbao.core.enums;


/**
 * 领取状态枚举
 *
 * @author yhh
 * @E-Mail dingl@163.com
 * @create 2018-05-19 下午1:09
 **/
public enum CouponTypeEnums {

    COUPON_TYPE_RED(1,"红包"),//1:红包 2:代金 3:兑换',
    COUPON_TYPE_EXCHANGE_COUPON(2,"代金"),
    COUPON_TYPE_CASH_COUPON(3,"兑换");

    /****
     * code
     */
    private Integer code;
    /****
     * 说明
     */
    private String desc;

    public Integer getCode() {
        return code;
    }

    public void setCode(Integer code) {
        this.code = code;
    }

    public String getDesc() {
        return desc;
    }

    public void setDesc(String desc) {
        this.desc = desc;
    }

    CouponTypeEnums(Integer code, String desc) {

        this.code=code;
        this.desc=desc;
    }
    public static Integer getCode(String desc){

        for(CouponTypeEnums receiveOpenEnums:CouponTypeEnums.values()){
            if(receiveOpenEnums.getDesc().equals(desc)){
                return receiveOpenEnums.getCode();
            }
        }
        return null;
    }
    public static String getDesc(Integer code){

        for(CouponTypeEnums receiveOpenEnums:CouponTypeEnums.values()){
            if(receiveOpenEnums.getCode().equals(code)){
                return receiveOpenEnums.getDesc();
            }
        }
        return null;
    }

    public static void main(String[] args) {
        String a = CouponTypeEnums.getDesc(1);
        System.out.println(a);
    }
}